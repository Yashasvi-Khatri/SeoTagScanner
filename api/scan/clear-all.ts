...
import { clearAllScans } from '../../lib/scanModel.js';

export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log('=== CLEAR ALL SCANS API REQUEST ===')
  ...
  try {
    ...
    console.log('Step 3: Clearing all scans from database')
    await clearAllScans(decoded.userId);
    console.log('All scans cleared successfully')

    console.log('=== CLEAR ALL SCANS API SUCCESS ===')
    return res.status(200).json({ message: "All scans cleared successfully" });
  } catch (err: any) {
    console.error('=== CLEAR ALL SCANS API ERROR ===')
    ...
    return res.status(500).json({ message: "Failed to clear scans" });
  }
}
